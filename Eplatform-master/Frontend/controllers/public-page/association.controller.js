...
function getCompany(associationId, lang) {
    var deferred = Q.defer();
    let promises = [
        systemService.getList(collectionConst.COMPANIES, {}, lang, 12, { "associationId.id" : typeCast.castObjectId(associationId) })   
    ]
    Q.all (promises)
        .then (function(arr) {
            let promises2 = [];
...
function getAssociationData(url, lang) {
    var deferred = Q.defer();
    var data = {};

    let promises = [
        systemService.getByUrl(collectionConst.ASSOCIATIONS, url, lang)
    ]
    Q.all (promises)
        .then (function(arr) {
            data = arr[0];
            let promises2 = [
                ...
                systemService.getList(collectionConst.COMPANYNEWS, {}, lang, undefined, { "associationId.id" : typeCast.castObjectId(arr[0]._id) })
            ]
            Q.all(promises2)
                .then (function(array) {
...
function Controller (req, res, next) {
    ...
    for (lang of config.langs) {
        try {
            url = req.path.match(treeMenu.association[lang].regex)[treeMenu.association[lang].param.url];
            break;
        } catch (ex) {};
    }

    let promises = [
        getAssociationData(url, req.language)
    ]
    Q.all (promises)
        .then(function (obj) {
            MyRender(req, res, "association", obj[0]);
        })
        .catch(function(err){
            console.log(err);
            next();
